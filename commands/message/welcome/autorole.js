...
majdoor += `\`[${i+1}]\`  -  ${role}\n`
          ...
          return message.channel.send({embeds: [new client.emb().setAuthor({name:`Flare Autoroles`, iconURL: message.guild.iconURL({dynamic: true})}).setFooter({text: `Requested By `+ message.author.username, iconURL: message.author.displayAvatarURL({dynamic: true})}).addFields([{name: `Human Autoroles`, value: insaan.length > 0 ? insaan : `None`}, {name: `Bot Autoroles`, value: majdoor.length > 0 ? majdoor : `None`}])]})
