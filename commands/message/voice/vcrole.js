...
majdoor += `\`[${i+1}]\`  -  ${role}\n`
          ...
          return message.channel.send({embeds: [new client.emb().setAuthor({name:`Flare Vcroles`, iconURL: message.guild.iconURL({dynamic: true})}).setFooter({text: `Requested By `+ message.author.username, iconURL: message.author.displayAvatarURL({dynamic: true})}).addFields([{name: `Human Vcroles`, value: insaan.length > 0 ? insaan : `None`}, {name: `Bot Vcroles`, value: majdoor.length > 0 ? majdoor : `None`}])]})
