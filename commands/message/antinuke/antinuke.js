...
module.exports = {
  ...
  execute: async (client, message, args, prefix) => {
	...
	permissionOverwrites: [
      ...
      ch.send(`**This Channel Is Specifically Configured For Flare's Antinuke Logs**`)
      return message.channel.send({embeds: [new client.emb().setAuthor({name: message.author.tag, iconURL: message.author.displayAvatarURL({dynamic: true})}).title(`Flare Security`).desc(`**${client.emoji.tick} The antinuke setup has been completed.**\n\n*Move my role above for the antinuke to work properly*\n\nLogging Channel: ${ch}\nPunishment: Ban`).setTimestamp().setFooter({text: message.guild.name, iconURL: message.guild.iconURL({dynamic: true})})]})
