...
module.exports = {
  ...
  for (var i = 0; i < str.length; i++) {
    ...
    let ticket = new EmbedBuilder().setColor(`#2f3136`).addFields({ name: `__Tickets__`, value: "`ticket setup`, `ticket config`, `ticket reset`" }).setAuthor({name: message.author.tag, iconURL: message.author.displayAvatarURL({dynamic: true})})
    ...
    let ven = new EmbedBuilder().setColor(`#2f3136`).addFields({ name: `__Vanity Roles__`, value: "`vanityroles`, `vanityroles setup`, `vanityroles setvanity`, `vanityroles reset`" }).setAuthor({name: message.author.tag, iconURL: message.author.displayAvatarURL({dynamic: true})})
