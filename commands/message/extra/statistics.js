...
module.exports = {
  ...
  execute: async (client, message, args, prefix) => {
    ...
    .desc(`**[Flare Development](${client.invite})**`)
    .setAuthor({name: client.user.username, iconURL: client.user.displayAvatarURL({dynamic: true})})
    .setFooter({text: `Requested by ${message.author.username}`, iconURL: message.author.displayAvatarURL({dynamic: true})})
    .addFields([
      {
        name: `__Flare Info__`,
        value: `**Servers**: ${client.guilds.cache.size}\n**Users**: ${client.guilds.cache.reduce((acc, guild) => acc + guild.memberCount, 0)} | **Cached**: ${client.users.cache.size}\n**Websocket Latency**: ${client.ws.ping}\n**Last Reboot**: <t:${Math.round((Date.now() - client.uptime)/1000)}:R>\n**Total Commands**: 142`
      },
      {
        name: `__Channels__`,
        value: `<:channel:1120932317792309288> ${client.channels.cache.filter(c => c.type === 0).size} | <:voice:1120932433890652200> ${client.channels.cache.filter(c => c.type === 2).size} | <:flare_stage:1087364653178421349> ${client.channels.cache.filter(c => c.type === 13).size}`
      },
      {
