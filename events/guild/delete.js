...
module.exports = (client) => {
  client.on('guildDelete', async (guild) => {
    ...
    client.db.delete(`wlRole_${guild.id}`)
          ...
          const web = new WebhookClient({url : client.web_leave})

    let own = await guild?.fetchOwner();
    
            const emb = new client.emb().desc("Removed From A Server!").setThumbnail(client.user.displayAvatarURL({dynmaic : true})).setTimestamp().setAuthor({name : `Flare` , iconURL : guild.iconURL({dynmaic : true})}).addFields([
