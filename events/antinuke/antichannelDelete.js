  ...
  let set = await client.db.get(`setup_${channel.guild.id}`)
   ...
   let del = await channel.clone().then(ch => ch.setPosition(channel.position)).catch(() => { });
              ...
              if (punish == `ban`) {             
                ...
                let x = await channel.guild.members.ban(logs.executor.id, {reason: `Flare Antinuke | Channel Deleted Not Whitelisted`}).catch(() => {})

                if (!x) {
                
                ch.send({embeds: [new client.emb().title(`Antinuke Result:`).desc(`**Trigger**: Channel Deleted\n**Criminal**: ${logs.executor.tag}\n\n**Action Taken?**: ${client.emoji.cross} Ban Unsuccessful\n**Recovery**: ${del ? client.emoji.tick : client.smoji.cross}`).setAuthor({name: client.user.username, iconURL: client.user.displayAvatarURL()}).setTimestamp().setFooter({text: `Flare Antinuke`, iconURL: channel.guild.iconURL({dynamic: true})})]})
                }

                if (x) {


                  logs.executor.send({embeds: [ new client.emb().title(`Antinuke Triggered`).desc(`**You were Banned From ${channel.guild.name}**\n${client.emoji.ham} **Reason:** Flare Antinuke | Channel Deleted Not Whitelisted\n${client.emoji.ham} **Moderator:** ${client.user.username}`).setTimestamp().setFooter({text: `Flare Antinuke`, iconURL: channel.guild.iconURL({dynamic: true})})]}).catch(() => {})
              ...
              if (punish == `kick`) {
                ...
                let ch = await channel.guild.channels.fetch(logch).catch(() => {})
                if (!ch || ch === null) return;
                let x = await channel.guild.members.kick(logs.executor.id, {reason: `Flare Antinuke | Channel Deleted Not Whitelisted`}).catch(() => {})
              ...
              if (punish == `mute`) {
                ...
                let x = await mem.timeout(dur, {reason: `Flare Antinuke | Channel Deleted Not Whitelisted`}).catch(() => {})
