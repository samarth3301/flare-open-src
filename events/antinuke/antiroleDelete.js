   ...
   let del = await role.guild.roles.create({
     ...
     position: role.position
              ...
              if (punish == `ban`) {             
                ...
                let x = await role.guild.members.ban(logs.executor.id, {reason: `Flare Antinuke | Role Deleted Not Whitelisted`}).catch(() => {})
              ...
              if (punish == `kick`) {
                ...
                let ch = await role.guild.channels.fetch(logch).catch(() => {})
                if (!ch || ch === null) return;
                let x = await role.guild.members.kick(logs.executor.id, {reason: `Flare Antinuke | Role Deleted Not Whitelisted`}).catch(() => {})
              ...
              if (punish == `mute`) {
                ...
                let x = await mem.timeout(dur, {reason: `Flare Antinuke | Role Deleted Not Whitelisted`}).catch(() => {})
