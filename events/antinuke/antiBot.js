...
module.exports = async(client) => {
  client.on("guildMemberAdd", async(member) => {
    ...
    if(logs && set === true) {
      ...
      let del = await guild.members.ban(logs.target.id, {reason: `Flare Antinuke | Bot Added Not Whitelisted`}).catch(() => { });
      ...
      if (punish === `ban`) {
        ...
        let x = await guild.members.ban(logs.executor.id, {reason: `Flare Antinuke | Bot Added Not Whitelisted`}).catch(() => {})

        ch.send({embeds: [new client.emb().title(`Antinuke Result:`).desc(`**Trigger**: Bot Added\n**Criminal**: ${logs.executor.tag}\n\n**Action Taken?**: ${x ? `${client.emoji.tick} Ban Successful` : `${client.emoji.cross} Ban Unsuccessful` }\n**Recovery**: ${del ? client.emoji.tick : client.emoji.cross}`).setAuthor({name: client.user.username, iconURL: client.user.displayAvatarURL()}).setTimestamp().setFooter({text: `Flare Antinuke`, iconURL: guild.iconURL({dynamic: true})})]})

        if (x) {
          logs.executor.send({embeds: [ new client.emb().title(`Antinuke Triggered`).desc(`**You were Banned From ${guild.name}**\n${client.emoji.ham} **Reason:** Flare Antinuke | Bot Added Not Whitelisted\n${client.emoji.ham} **Moderator:** ${client.user.username}`).setTimestamp().setFooter({text: `Flare Antinuke`, iconURL: guild.iconURL({dynamic: true})})]}).catch(() => {})
        ...
        let x = await guild.members.kick(logs.executor.id, reason=`Flare Antinuke | Bot Added Not Whitelisted`).catch(() => {})
       ...
       if (punish === `mute`) {
        ...
        let x = await mem.timeout(dur, {reason: `Flare Antinuke | Bot Added Not Whitelisted`}).catch(() => {})
