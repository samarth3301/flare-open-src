  ...
  client.on('messageCreate', async (message) => {
    ...
    if (message.mentions.everyone || message.mentions.here) {
      ...
      await message.member.timeout(900000).then(() => {
        ...
        ch.send({embeds: [new client.emb().desc(`**${client.emoji.warn} Member Muted**\n**Member**: ${message.author.tag}\n**Member Id**: ${message.author.id}\n\n**Reason**: Attempted \`@everyone / @here\``).setAuthor({name: `Automod Action`, iconURL: message.author.displayAvatarURL({dynamic: true})}).setFooter({text: `Flare Automod`, iconURL: client.user.displayAvatarURL()}).setTimestamp()]}).catch(() => { })
    ...
    if (recentMessages2.length > 3 && similarity > 70) {
      let x = await message.member.timeout(900000)
        .then(() => {
          ...
          ch.send({embeds: [new client.emb().desc(`**${client.emoji.warn} Member Muted**\n**Member**: ${message.author.tag}\n**Member Id**: ${message.author.id}\n\n**Reason**: Spamming`).setAuthor({name: `Automod Action`, iconURL: message.author.displayAvatarURL({dynamic: true})}).setFooter({text: `Flare Automod`, iconURL: client.user.displayAvatarURL()}).setTimestamp()]})
